...
import { CalibrationGovernance } from "./calibration-governance";
...
export default async function UnderwritingPage({
  ...
  const providerScorecards = calibration?.provider_scorecards ?? [];
  const calibrationDecisions = calibration?.decisions ?? [];
  ...
  return (
    <WorkspacePage>
      ...
      <section className={styles.section}>
        <header>
          <div><span>Market proof</span><h3>Provider and methodology scorecard</h3></div>
          <strong>{calibration?.markets.length ?? 0} markets</strong>
        </header>
        <div aria-label="Underwriting market scorecard" className={styles.tableWrap} tabIndex={0}>
          <table>
            ...
            <tbody>
              {providerScorecards.length ? providerScorecards.map((market) => (
      ...
      <section className={styles.section}>
        ...
        <div aria-label="Verified outcome history table" className={styles.tableWrap} tabIndex={0}>
          <table><thead><tr><th>Property</th><th>Market</th><th>Evidence</th><th>Provider</th><th>Predicted ARV</th><th>Verified ARV</th><th>Error</th><th>Range</th></tr></thead>
            <tbody>
              {calibration?.cases.length ? calibration.cases.map((item) => <tr key={item.id}><td><Link href={`/os/leads/${item.lead_id}#underwriting`}>{item.property_address}</Link><small>{item.seller_name}</small></td><td>{item.market_key}</td><td>{labelize(item.benchmark_type)}</td><td>{labelize(item.provider)}<small>{item.methodology_version ?? "Unversioned"} / {item.confidence_score}% confidence</small></td><td>{money(item.predicted_arv_point_cents)}</td><td>{money(item.benchmark_arv_cents)}</td><td>{percent(item.arv_error_percentage, true)}</td><td><StatusBadge tone={item.arv_range_hit ? "success" : item.arv_range_hit === false ? "danger" : "neutral"}>{item.arv_range_hit === null ? "No range" : item.arv_range_hit ? "Inside" : "Outside"}</StatusBadge></td></tr>) : <tr><td colSpan={8}>No verified outcomes have been recorded.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
