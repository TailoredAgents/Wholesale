  ...
  return (
    <section aria-label="Buyer management" className={styles.workspace}>
      ...
      <nav aria-label="Buyer lifecycle" className={styles.segmentNav}>
        <Link aria-current={segment === "leads" ? "page" : undefined} className={segment === "leads" ? styles.activeSegment : ""} href="/os/buyers?segment=leads"><span>Active Buyer Prospects</span><small>Investors currently being worked for a property</small></Link>
      ...
      <section className={styles.metrics} aria-label="Buyer network summary"><div><UsersRound size={17} /><span>{segment === "leads" ? "Matching prospects" : segment === "past" ? "Past buyers" : "Matching buyers"}</span><strong>{total}</strong></div><div><ShieldCheck size={17} /><span>Active on page</span><strong>{activeOnPage}</strong></div><div><Building2 size={17} /><span>Deals needing buyers</span><strong>{contractLeads.length}</strong></div><div><BadgeDollarSign size={17} /><span>Expired POF on page</span><strong>{expiredOnPage}</strong></div></section>
      ...
      <form className={styles.toolbar} onSubmit={submitFilters} role="search">
        ...
        {canEdit ? <button className={styles.addButton} onClick={() => setShowCreate(true)} type="button"><Plus size={15} />{segment === "leads" ? "Add buyer prospect" : "Add buyer"}</button> : null}
      </form>
      ...
      <Drawer description="Create a buyer prospect first. Connect them to a property when you begin working that opportunity, and add them to Buyer Network when the relationship is reusable." onClose={() => setShowCreate(false)} open={showCreate} title={segment === "leads" ? "Add buyer prospect" : "Add buyer"}><BuyerForm onCancel={() => setShowCreate(false)} onSaved={(saved) => { setShowCreate(false); setSelectedId(saved.id); router.push(returnTo ?? `/os/buyers?segment=leads&buyer=${saved.id}&tab=summary`); }} onUseExisting={useExisting} relationshipOwners={relationshipOwners} sourceOptions={sourceOptions} /></Drawer>
