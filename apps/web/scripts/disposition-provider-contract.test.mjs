...
test("DS8 keeps the InvestorLift handoff inside the canonical disposition workspace", () => {
  ...
  assert.match(disposition, /item === "provider" \? "InvestorLift"/);
  assert.match(disposition, /tab === "provider"/);
...
test("the UI states the manual-only unverified provider boundary", () => {
  for (const text of [
    ...
    "Only the current, human-approved Stonegate public package can be released.",
...
test("manual publication is exact-package, approval, and bundle gated", () => {
  for (const step of [
    "Prepare",
  ...
  assert.match(provider, /!data\.approved_package\?\.is_current/);
