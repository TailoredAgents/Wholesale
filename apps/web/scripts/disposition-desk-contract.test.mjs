...
test("the canonical disposition desk is URL-backed and server-loaded", () => {
  ...
  assert.match(api, /details_loaded: boolean/);
  assert.match(api, /deferred_sections: DispositionDeskCategory\[\]/);
...
test("the desk centers the two daily jobs and keeps supporting queues secondary", () => {
  ...
  assert.match(desk, /detailedData\.buyer_network\.missing_proof/);
  assert.match(desk, /detailedData\.coverage_warnings/);
  assert.match(desk, /<details className=\{styles\.cardChecklist\}[\s\S]*?>/);
  assert.match(desk, /Open details to load current readiness/);
  assert.match(desk, /\/api\/v1\/dispositions\/cases\/\$\{encodeURIComponent\(caseId\)\}\/readiness/);
  assert.match(desk, /if \(event\.currentTarget\.open && !caseWorkbench\)/);
  ...
  assert.match(desk, /<details className=\{styles\.deskDetails\}[\s\S]*?>/);
  assert.match(desk, /if \(event\.currentTarget\.open && !detailedData\)/);
  assert.match(desk, /\/api\/v1\/dispositions\/desk\?\$\{new URLSearchParams\(\{ scope: data\.effective_scope \}\)\}/);
  assert.match(desk, /return value === null \? "—" : value/);
...
test("stale, unavailable, and truncated data remain explicit without disabling canonical work", () => {
  ...
  assert.match(desk, /detailedData\.source_health\.external_provider_status/);
