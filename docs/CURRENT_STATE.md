...
- Scheduled GitHub production-readiness monitoring for API, worker heartbeat, and required public
  pages.
- Optional Sentry reporting for web, API, and worker errors with default PII, request-body, and
  Python local-variable capture disabled.
...
| Resend operational email | F8.1-F8.5 provider-neutral foundation, aliases, grants, outbound delivery, signed inbound processing, durable attachments, lifecycle reconciliation, recovery, authorized Inbox senders, owner administration, and routing-exception review are complete; production remains disabled | Complete F8.6 DNS, credentials, and webhook registration, then acceptance and launch |
...
- AI evaluation datasets still need redacted operating examples and measured model pilots. Agents
  ...
  legal decisions.
- AI1 is implemented: eight human-owned role copilots, 15 versioned capability contracts,
...
- A production PostgreSQL backup restored successfully into isolated local database
  `stonegate_prod_restore_verify_20260724` on July 24, 2026; migration, organization, and lead
  verification passed.
...
- Credential rotation, MFA rollout, and secret-security remediation were explicitly excluded from
  the current F1 execution by owner direction and remain known risks.
