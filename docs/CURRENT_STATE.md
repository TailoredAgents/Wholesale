...
| Resend operational email | F8.1-F8.6 and F8.7A.1 are complete; the branded API domain, sending and receiving DNS, credentials, signed webhook, generalized conversation contexts, and structured email participants are implemented; controlled transport is enabled | Build F8.7A.2 deterministic routing, finish the mailbox upgrade and F8.7 acceptance, then approve F8.8 production launch |
