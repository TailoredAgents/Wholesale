...
| **Record a signed amendment** | Adds a separately preserved executed addendum and changes the official purchase basis on the Transaction and Deal without replacing the original agreement | Available for executed or closing House and Land transactions; requires the current price to still match the value shown when the form opened |
| Revised purchase price | Records the newly binding seller price from the signed amendment | Must be positive and different from the current official price |
| Investor asking price choice | Keeps the current marketed price by default or records an explicit new asking price | The asking price cannot be below the revised purchase basis; keeping it recalculates the desired assignment spread automatically |
| Amendment evidence and attestation | Uploads the exact signed PDF, execution source/time, optional external reference, and a human verification note | Requires a PDF up to 15 MB, acceptable scan state, every required signature, and a 10-500 character verification note |
| Packet safety after amendment | Returns the Disposition package to Draft, clears approval, supersedes stale draft/approved versions, and revokes active packet links | Prevents investors from receiving a packet whose private economics no longer match the contract; the case, outreach history, and Under Contract stage remain intact |
