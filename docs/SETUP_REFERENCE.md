...
**Text all inbound messages** is a separate employee preference that uses the same live Twilio
staff alert delivery mode. For each seller or buyer text, Stonegate sends one internal alert to
every active, opted-in employee with a valid cellphone. The alert identifies the contact and links
to Inbox but does not copy the customer's message onto a personal phone. A previously unknown
number creates a reviewable seller lead on Acquisitions or buyer thread on Dispositions; STOP,
START, HELP, and messages from configured staff cellphones do not create records or notification
loops.
...
`staff_inbound_sms_alert_queue_evaluated`. Correct the **Text all inbound messages** preference or
cellphone when an employee is not included in the eligible recipient group.
